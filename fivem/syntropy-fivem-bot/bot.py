...
def start_fivem(ctx, players):
    ...
    for player in players:
        agent_ids.append(AgentsPairObject(players_endpoints[player].agent_id, fivem_endpoint.agent_id))
    
    time.sleep(5)

    api_mgr.remove_all_connections(fivem_endpoint.agent_id)
    connections_ids = api_mgr.add_connections(agent_ids, fivem_endpoint.agent_id)

    for player in players:
        connection_id = connections_ids[players_endpoints[player].agent_id]
        services = api_mgr.get_services([connection_id]) 
        subnet_id = get_subnet_id(services, "fivem", fivem_endpoint.agent_id)
